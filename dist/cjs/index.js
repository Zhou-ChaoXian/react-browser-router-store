    ...
    if (mustTrue.has(type)) {
      ...
      if (reducer) {
        ...
        const result = typeof reducer === "function" ?
          immer.produce(reducer).apply(undefined, args) :
          immer.produce(reducer.handle).apply(undefined, args);
...
function Transition(
  ...
  const [el, setEl] = react.useState(children);
...
const Context$2 = react.createContext(false);
...
function Keepalive(
  ...
  const sortedKeysList = react.useRef(new List()).current;
  const map = react.useRef(new Map()).current;
  const activeKey = react.useRef(uniqueKey);
  const isCache = react.useRef(true);
  if (!isCache.current) {
  ...
  if (map.has(uniqueKey)) {
    ...
    data.element = react.cloneElement(data.element);
    data.isActive = true;
    sortedKeysList.pushLast(data.node);
  } else {
    const node = sortedKeysList.addItem(uniqueKey);
    const element = react.isValidElement(children) ? children : react.createElement(react.Fragment, null, children);
  ...
  return [...sortedKeysList].map(({item: key}) => {
    ...
    return react.createElement(
      ...
      react.createElement(Context$2.Provider, {value: isActive}, element)
    );
  });
}

function useActivated(activatedHandle) {
  const isActive = react.useContext(Context$2);
  const deactivatedHandle = react.useRef(null);
  react.useLayoutEffect(() => {
    if (isActive) {
...
function Redirect({path, options, children}) {
  ...
  const {search, hash} = reactRouterDom.useLocation();
  const pathname = reactRouterDom.generatePath(path, params);
  react.useLayoutEffect(() => {
...
function Show({resolve, loading = false, error = (_) => false, onStart, onEnd, children}) {
  ...
  return react.createElement(ErrorBoundary, {fallback: error},
    react.createElement(react.Suspense, {fallback: loading}, react.createElement(ResolveInner, {resolve, children, onEnd}))
  );
}
...
function ShowList({loading = false, timeout = 0, delay = 300, onStart, onEnd, children}) {
  ...
  return react.createElement(react.Suspense, {fallback: loading}, react.createElement(ResolveInner, {resolve, children: handle, onEnd}));
}
...
function lazy(factory, name = undefined, loading = false, error = false) {
  ...
  return function LazyComponent(props) {
    ...
    return react.createElement(react.Suspense, {fallback: loading},
      react.createElement(reactRouterDom.Await, {resolve, errorElement: error}, module => {
        if (component === null) {
...
function lazyWithStore(factory, name = undefined) {
  return Object.defineProperties(function LazyComponentWithStore(props) {
    return react.createElement(Reflect.get(LazyComponentWithStore, "component"), props);
  }, {
  ...
  routes.forEach(item => {
    ...
    if (item.path !== undefined) {
      if (meta[redirect]) {
        ...
        item.element = react.createElement(Route, {route: composeComponent(wrapperList)(element)});
        if (meta[alias]) {
...
function composeComponent(componentWithPropsList) {
  ...
  return element => reverse.reduce((prev, current) => {
    ...
    return react.createElement(component, props, prev);
  }, element);
}
...
function handleRouteComponent(item) {
  ...
  if (component instanceof ComponentWithStore) {
    judgeUseKeepalive.flag = false;
    element = react.createElement(component.component);
    item.meta ??= {};
...
function handleRouteMetaRedirect(route) {
  ...
  route.element = react.createElement(Route, {route: react.createElement(Redirect, {path, options, children})});
}
...
function Route({route}) {
  const {handles: [before, after], router, globalOptions, handleNavigateGuard} = react.useContext(Context$1);
  ...
  const [element, setElement] = react.useState(() => globalOptions[start]);
  const routeRef = react.useRef();
  const to = reactRouterDom.useLocation(), from = react.useRef(to), fromPro = react.useRef(to);
  to.pathname === pathname && (to.meta ??= meta);
  const navigate = reactRouterDom.useNavigate();
  const resolveValue = react.useRef();
  const [isUseKeepaliveFlag] = react.useState(() => globalOptions[isUseKeepAlive]);
  react.useLayoutEffect(() => {
    let flag = true;
    ...
    before.reduce(
      ...
      const nextElement = react.createElement(RouteContext.Provider, {value: {pathname, store, meta}}, react.createElement(
        Transition, {...(meta[transition] ?? globalOptions[transition] ?? {}), uniqueKey},
        react.createElement(Keepalive, keepAliveProps, meta[enhancerFactory](() => route)({to, from: from.current, pathname}))
      ));
...
const routerFactory = {factory: reactRouterDom.createBrowserRouter};
...
function createBrowserRouter(routes, options = undefined, globalOptions = {}) {
  setRouterFactory(reactRouterDom.createBrowserRouter);
...
function createHashRouter(routes, options = undefined, globalOptions = {}) {
  setRouterFactory(reactRouterDom.createHashRouter);
  return baseCreateBrowserRouter(routes, options, globalOptions);
}
...
function baseCreateBrowserRouter(routes, options = undefined, globalOptions = {}, enhancer = undefined) {
  ...
  function useRouteMeta() {
    return react.useRef(react.useContext(RouteContext).meta).current;
  }

  function useNavigateGuard() {
    const pathname = react.useRef(react.useContext(RouteContext).pathname).current;
    const {current, subscribes} = navigateGuard;
    const change = react.useCallback(onStoreChange => {
    ...
    return react.useSyncExternalStore(change, () => current.active && current.pathname === pathname);
  }

  function useRouterStore() {
    const store = react.useRef(react.useContext(RouteContext).store).current;
    ...
    react.useSyncExternalStore(change, () => store.state);
    if (!store.trigger) {
  ...
  function useRouterStoreState() {
    ...
    const setStore = react.useCallback(function setStore(handle) {
      typeof handle === "function" && reset(immer.produce(handle)(store.state, detail, options));
    }, []);
  ...
  function useRouterStoreStatePro__base(store, getHandle) {
    const {state, detail, options, reset} = store;
    const dispatch = react.useCallback(function dispatch(action) {
      if (typeof action === "function")
        return action(dispatch);
      const handle = getHandle(action);
      if (typeof handle === "function") {
        reset(immer.produce(handle)(store.state, action, detail, options));
        return action;
  ...
  function useRouterStoreReducer(reducersOrFactory) {
    const store = useRouterStore();
    const [reducers] = react.useState(() => typeof reducersOrFactory === "function" ?
  ...
  function RouterProvider({router: __router, fallbackElement, future}) {
    const realRouter = react.useSyncExternalStore(onStoreChange => {
    ...
    return react.createElement(Context$1.Provider, {value}, react.createElement(reactRouterDom.RouterProvider, {
      router: realRouter,
      fallbackElement,
      future,
    }));
  ...
  return {
    ...
    hasRoute: (locationArg, basename = "/") => {
      const matches = reactRouterDom.matchRoutes(router.routes, locationArg, basename);
...
function useRouter() {
  return react.useContext(Context$1)["__router"];
}
...
function useRouterStore() {
  return react.useContext(Context).storeInfo;
}

function useRouterHooks() {
  return react.useContext(Context).routerHooks;
}
...
function routerNativeHooks(component, useRouterNativeHooksHandle = defaultUseRouterNativeHooksHandle) {
  return function RouterNativeHooks() {
    ...
    return react.useState(() => generateElement(component, undefined, routerHooks))[0];
  }
}
...
function generateElement(component, storeInfo, routerHooks) {
  return react.createElement(Context.Provider, {value: {storeInfo, routerHooks}}, react.createElement(component));
}

function useHook(component, storeInfo, state, router, useRouterHooksHandle) {
  const routerHooks = useRouterHooksHandle(router);
  const [element, setElement] = react.useState(() => generateElement(component, storeInfo, routerHooks));
...
exports.createReducerHandle = createReducerHandle;
exports.createRouterStore = createRouterStore;
exports.enhancer = enhancer;
exports.guardError = guardError;
exports.keepalive = keepalive;
exports.lazy = lazy;
exports.lazyWithStore = lazyWithStore;
exports.makeCallback = makeCallback;
exports.makeEffect = makeEffect;
exports.makeMemo = makeMemo;
exports.max = max;
exports.noCache = noCache;
exports.redirect = redirect;
exports.registerGlobalPlugin = registerGlobalPlugin;
exports.routerNativeHooks = routerNativeHooks;
exports.routerStore = routerStore;
exports.routerStoreCompose = routerStoreCompose;
exports.routerStoreReducer = routerStoreReducer;
exports.routerStoreState = routerStoreState;
exports.start = start;
exports.store = store;
exports.transition = transition;
exports.useActivated = useActivated;
exports.useRouter = useRouter;
exports.useRouterHooks = useRouterHooks;
